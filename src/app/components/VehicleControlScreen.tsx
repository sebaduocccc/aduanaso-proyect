...
import { Car, Printer, CheckCircle, Globe, Search, Clock } from "lucide-react";
...
function Countdown({ days, startDate }: { days: number; startDate: string }) {
  ...
  const barColor = remaining > 30 ? "#16A34A" : remaining > 7 ? "#D97706" : "#C8102E";
    ...
    <div className="rounded p-4" style={{ background: "var(--muted)", border: "1px solid var(--border)" }}>
      <div className="flex items-center justify-between mb-2">
        <div className="flex items-center gap-2">
          ...
          <span style={{ fontSize: "11px", fontWeight: 700, color: "var(--foreground)" }}>Contador de Validez Automático</span>
        </div>
        <span style={{ fontSize: "11px", fontFamily: "JetBrains Mono, monospace", fontWeight: 700, color: barColor }}>
  ...
  return (
    <div className="flex-1 overflow-y-auto p-6" style={{ background: "var(--background)" }}>
      ...
      <div className="flex items-center justify-between mb-5">
        <div>
          <h1 style={{ color: "var(--foreground)", fontSize: "18px", fontWeight: 700 }}>Control de Vehículos Internacionales</h1>
      ...
      {/* DB Integration Bar */}
      <div className="rounded p-4 mb-5" style={{ background: "var(--sidebar)", border: "1px solid var(--sidebar-border)" }}>
      ...
      <div className="grid grid-cols-3 gap-5">
        ...
        <div className="col-span-2 flex flex-col gap-4">
          ...
          <div className="rounded p-5" style={{ background: "var(--card)", border: "1px solid var(--border)" }}>
            <div style={{ fontSize: "12px", fontWeight: 700, color: "var(--foreground)", marginBottom: "12px" }}>TIPO DE VEHÍCULO</div>
            <div className="grid grid-cols-2 gap-3 mb-4">
              {[
                ...
                <button
                  key={t.value}
                  onClick={() => { setVehicleType(t.value as VehicleType); setPlateType(t.value === "diplomatico" ? "CD" : "REGULAR"); }}
                  className="flex items-start gap-3 px-4 py-4 rounded text-left transition-all"
                  style={{
                    border: `2px solid ${vehicleType === t.value ? "#2563EB" : "var(--border)"}`,
                    background: vehicleType === t.value ? "rgba(37,99,235,0.08)" : "var(--muted)",
                    cursor: "pointer",
                  }}
                >
                  <span style={{ fontSize: "24px" }}>{t.icon}</span>
                  <div>
                    <div style={{ fontSize: "13px", fontWeight: 700, color: "var(--foreground)" }}>{t.label}</div>
                    <div style={{ fontSize: "10px", color: "var(--muted-foreground)" }}>{t.desc}</div>
                  </div>
                </button>
              ))}
            </div>

            {vehicleType === "diplomatico" && (
              <div>
                <div style={{ fontSize: "11px", fontWeight: 700, color: "var(--muted-foreground)", marginBottom: "8px", letterSpacing: "0.05em" }}>
                ...
                <div className="flex gap-2 flex-wrap">
                  {(["CD", "CC", "OI", "PAT"] as PlateType[]).map(p => (
                    <button
                      ...
                      style={{
                        border: `1.5px solid ${plateType === p ? "#7C3AED" : "var(--border)"}`,
                        background: plateType === p ? "rgba(124,58,237,0.1)" : "var(--muted)",
                        color: plateType === p ? "#7C3AED" : "var(--muted-foreground)",
                        fontSize: "12px",
                        fontWeight: 700,
                        cursor: "pointer",
          ...
          {/* Vehicle data */}
          <div className="rounded p-5" style={{ background: "var(--card)", border: "1px solid var(--border)" }}>
            <div style={{ fontSize: "12px", fontWeight: 700, color: "var(--foreground)", marginBottom: "12px" }}>DATOS DEL VEHÍCULO</div>
            <div className="flex gap-2 mb-4">
              <input
                ...
                style={{ border: "1px solid var(--border)", background: "var(--input-background)", fontSize: "13px", fontFamily: "JetBrains Mono, monospace", color: "var(--foreground)", letterSpacing: "0.08em" }}
              />
              <select value={pais} onChange={e => setPais(e.target.value)} className="px-3 py-2.5 rounded outline-none"
              ...
              <button onClick={handleSearch} className="flex items-center gap-2 px-4 py-2 rounded"
            ...
            <div className="grid grid-cols-2 gap-3">
              {[
                ["Marca",  marca,    setMarca,    "Toyota, Ford..."],
                ["Modelo", modelo,   setModelo,   "Hilux, Ranger..."],
                ["Color",  colorVal, setColorVal, "Blanco, Negro..."],
                ["Año",    year,     setYear,     "2022"],
              ].map(([label, val, setter, ph]) => (
                <div key={label as string}>
                  <div style={{ fontSize: "10px", fontWeight: 700, color: "var(--muted-foreground)", marginBottom: "5px", letterSpacing: "0.06em" }}>{label as string}</div>
                  <input type="text" value={val as string} onChange={e => (setter as any)(e.target.value)} placeholder={ph as string}
            ...
            {searched && (
              <div className="mt-3 px-3 py-2 rounded flex items-center gap-2" style={{ background: "rgba(22,163,74,0.1)", border: "1px solid rgba(22,163,74,0.35)" }}>
                ...
                <span style={{ fontSize: "11px", color: "#16A34A", fontWeight: 600 }}>Vehículo encontrado en registros — sin alertas activas</span>
              </div>
            )}
          </div>
          ...
          {vehicleType && (
            <div className="rounded p-5" style={{ background: "var(--card)", border: "1px solid var(--border)" }}>
              ...
              <div className="mt-3 grid grid-cols-2 gap-3">
                {[
                  ["FECHA INGRESO",    new Date(entryDate).toLocaleDateString("es-CL")],
                  ["DÍAS AUTORIZADOS", `${validity} días corridos`],
                ].map(([k, v]) => (
                  <div key={k} className="px-3 py-2 rounded" style={{ background: "rgba(37,99,235,0.08)", border: "1px solid rgba(37,99,235,0.2)" }}>
                    <div style={{ fontSize: "9px", color: "var(--muted-foreground)", fontWeight: 700 }}>{k}</div>
                    <div style={{ fontSize: "12px", fontFamily: "JetBrains Mono, monospace", color: "var(--foreground)" }}>{v}</div>
                  </div>
                ))}
              </div>
            </div>
        ...
        <div className="flex flex-col gap-4">
          {/* Print */}
          <div className="rounded p-5" style={{ background: "var(--card)", border: "1px solid var(--border)" }}>
            <div style={{ fontSize: "12px", fontWeight: 700, color: "var(--foreground)", marginBottom: "12px" }}>PROCESAMIENTO FÍSICO</div>
            <div className="flex flex-col gap-2 mb-3">
              {["Copia Oficial (Aduana)", "Copia Titular (Conductor)"].map(label => (
                <div key={label} className="flex items-center justify-between px-3 py-2 rounded" style={{ background: "rgba(22,163,74,0.08)", border: "1px solid rgba(22,163,74,0.3)" }}>
                  <div className="flex items-center gap-2">
                    <CheckCircle size={12} color="#16A34A" />
                    <span style={{ fontSize: "11px", color: "#16A34A" }}>{label}</span>
                  </div>
                  <span style={{ fontSize: "10px", color: "#16A34A", fontFamily: "JetBrains Mono, monospace" }}>1 copia</span>
                </div>
              ))}
            </div>
            <button onClick={handlePrint} className="w-full flex items-center justify-center gap-2 py-3 rounded transition-all"
              style={{ background: printed ? "#16A34A" : "#1B3A6B", color: "#fff", fontSize: "13px", fontWeight: 700, cursor: "pointer", border: "none", letterSpacing: "0.04em" }}>
              <Printer size={15} />
              {printed ? "✓ IMPRESO (2 COPIAS)" : "IMPRIMIR COMPROBANTE DUAL"}
            </button>
            <p style={{ fontSize: "10px", color: "var(--muted-foreground)", textAlign: "center", marginTop: "8px" }}>
          ...
          {/* Recent vehicles */}
          <div className="rounded p-5" style={{ background: "var(--card)", border: "1px solid var(--border)" }}>
            <div style={{ fontSize: "12px", fontWeight: 700, color: "var(--foreground)", marginBottom: "12px" }}>ÚLTIMOS VEHÍCULOS</div>
            <div className="flex flex-col gap-1.5">
              {recentVehicles.map(v => (
                <div key={v.ppu} className="flex items-center justify-between px-3 py-2 rounded" style={{ background: "var(--muted)", border: "1px solid var(--border)" }}>
                  <div className="flex items-center gap-2">
                    <div className="w-1.5 h-1.5 rounded-full" style={{ background: v.estado === "ok" ? "#16A34A" : "#C8102E" }} />
                    <span style={{ fontSize: "11px", fontWeight: 700, color: "var(--foreground)", fontFamily: "JetBrains Mono, monospace" }}>{v.ppu}</span>
                    <span style={{ fontSize: "10px", color: "var(--muted-foreground)" }}>{v.pais} · {v.tipo}</span>
                  </div>
                  <span style={{ fontSize: "9px", color: "var(--muted-foreground)", fontFamily: "JetBrains Mono, monospace" }}>{v.hora}</span>
                </div>
              ))}
            </div>
          </div>
